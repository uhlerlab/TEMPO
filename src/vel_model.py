...
class MMFMModelGuidanceWrapper:
    """Wrapper for MMFM model with guidance.

    Implements sampling strategy proposed in the paper:
    Zheng, Qinqing, et al. "Guided flows for generative modeling and decision making." arXiv preprint arXiv:2311.13443 (2023).
    """

    def __init__(self, mmfm_model, guidance):
        self.mmfm_model = mmfm_model
...
class VectorFieldModel(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Initialize the VectorFieldModel."""
        super().__init__()
    ...
    def _get_normalization_layer(self, dim):
        """Helper function to get the normalization layer based on the specified type."""
...
class MultiVectorFieldModel(nn.Module):
    ...
    def _get_model_index(self, t, c):
        """Get the index of the appropriate VectorFieldModel instance based on the time indicator."""
    ...
    def forward(self, x):
        """Forward pass through the appropriate VectorFieldModel instance based on the time indicator."""
        data = x[:, :-2]
...
class SimpleModel(torch.nn.Module):
    """Simple model for piecewise condition and time vectorfields."""

    def __init__(self, dim, hidden=6, time_embed_dim=6):
        super().__init__()
        self.net = torch.nn.Sequential(
            torch.nn.Linear(dim + time_embed_dim, hidden),
            torch.nn.LeakyReLU(),
    ...
    def forward(self, x):
        ...
        # Make sure 2nd dimension is always 1 and t has 2 dimensions
        if t.dim() == 0:
            t = t[None]
        elif t.dim() == 1:
            pass

        t_out = timestep_embedding(t * 1000, self.time_embed_dim)

        x_total = torch.cat([x, t_out], dim=1)

        return self.net(x_total)


class MultiVectorFieldModelTCFM(nn.Module):
    """Wrapper model that stores N independent VectorFieldModel instances."""
    ...
    def __init__(
        ...
        self.mapping_idx_to_t = Dict()
        for i in range(1, len(self.model_list)):
            if self.model_list[i] == 1.0:
                self.mapping_idx_to_t[(self.model_list[i - 1], 1.01)] = i
            else:
                self.mapping_idx_to_t[(self.model_list[i - 1], self.model_list[i])] = i

    def _get_model_index(self, t):
        """Get the index of the appropriate VectorFieldModel instance based on the time indicator."""
        for (t1, t2), idx in self.mapping_idx_to_t.items():
            if t1 <= t < t2:
                return idx
        # if t > t2, return the last model
        if t >= t2:
            return idx
        raise ValueError(f"No model found for time {t}")
    ...
    def forward(self, x):
        ...
        idxs = []
        outputs = []
        for from_to_, model_id in self.mapping_idx_to_t.items():
            # Find all indices where the time is between from_ and to_
            idx = (from_to_[0] <= t) & (t < from_to_[1])
            if idx.sum() == 0:
                continue
